...
public class SecurityPoliciesController {
    ...
    public SecurityPoliciesController(DataPurgeService dataPurgeService) {
    ...
    public static class SecurityPolicySettings {
        ...
        public boolean isAutomaticDataPurge() {
            return automaticDataPurge;
        }

        public void setAutomaticDataPurge(boolean automaticDataPurge) {
            this.automaticDataPurge = automaticDataPurge;
        }
    ...
    public ResponseEntity<?> saveSecurityPolicies(@RequestBody SecurityPolicySettings settings) {
        try {
            ...
            // Если включено автоматическое удаление данных, обновляем соответствующие
            // настройки
    ...
    public ResponseEntity<SecurityPolicySettings> getSecurityPolicies() {
        try {
            // В реальном приложении здесь будет логика получения политик безопасности из
            // базы данных
        ...
        try {
            ...
            if (isValid) {
                return ResponseEntity.ok()
                        .body(Map.of("success", true, "message", "Security policies validation successful"));
            } else {
                return ResponseEntity.badRequest()
                        .body(Map.of("success", false, "message", "Security policies validation failed"));
            }
        } catch (Exception e) {
